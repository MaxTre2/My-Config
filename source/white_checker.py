...
# =============================================================================
# ГЛУБОКАЯ ПРОВЕРКА ТОП-КОНФИГОВ ЧЕРЕЗ XRAY (HANDSHAKE FIX)
# =============================================================================
...
def xray_fast_check(keys: list, limit: int = 150) -> list:
    ...
    dropped = len(keys_to_test) - len(valid_keys)
...
def main(dry_run=False):
    ...
    if not dry_run:
        res_ru_fast = xray_fast_check(res_ru_fast, limit=150)
        res_euro_fast = xray_fast_check(res_euro_fast, limit=150)
...
ИСПРАВЛЕНИЯ: Защита от зомби-процессов, точные WHITE-домены, таймауты
"""
...
def _stream_settings(params: dict, net: str, security: str, host: str) -> dict:
    ...
    if net == "ws":
        ...
        if max_bytes:
            try: xhttp_settings["maxBytes"] = int(max_bytes)
            except ValueError: pass
        if max_concurrency:
            try: xhttp_settings["maxConcurrency"] = int(max_concurrency)
            except ValueError: pass
        if min_upload_interval:
            try: xhttp_settings["minUploadInterval"] = int(min_upload_interval)
            except ValueError: pass
        if extra_json:
            try: xhttp_settings["extra"] = json.loads(extra_json)
