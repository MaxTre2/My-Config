...
# ИСПРАВЛЕНИЕ: Оставлены только надежные домены РФ. 
# Google/Cloudflare удалены, так как прокси с жесткой серверной маршрутизацией 
# на них отваливаются, отправляя рабочие конфиги в BLACK.
WHITE_TEST_DOMAINS = [
    "alfabank.ru",
    "mironline.ru", 
    "vkusvill.ru",
...
# ИСПРАВЛЕНИЕ: Таймаут снижен до 20 сек, так как мы проверяем только РФ домены
...
def _check_one(vpn_uri: str, timeout: float) -> bool:
    ...
    try:
        ...
        # ИСПРАВЛЕНИЕ: start_new_session=True для создания новой группы процессов
        ...
        # ИСПРАВЛЕНИЕ: Полное уничтожение процесса и всех его дочерних процессов
        if proc:
            try:
