...
Поддержка: VLESS REALITY+Vision, TLS+CDN+Vision, SplitHTTP/xhttp, gRPC, HTTPUpgrade.
Thread-safe, без глобального состояния, корректная обработка timeout и shutdown.
"""
...
HTTP_TIMEOUT      = 5       # ↓ 6→5 таймаут одного HTTP-запроса через SOCKS5
XRAY_STARTUP_WAIT = 5.0     # ↓ 6→5 ждём запуска xray
XRAY_POLL_INTERVAL = 0.10   # Интервал опроса порта
WHITE_CHECK_TIMEOUT = 20.0  # ↓ 25→20 общий таймаут одной проверки
WHITE_WORKERS     = 10      # ↑ 6→10 параллельных проверок (~600 сек = 10 мин на 150 ключей)
