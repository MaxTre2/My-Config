...
# ИСПРАВЛЕНИЕ 3: Убран жестко зашитый путь, оставлена только переменная окружения
...
def _xray_binary() -> Optional[str]:
    """Ищет бинарник xray: рядом со скриптом (source/) → переменная окружения → PATH."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 1. Ищем строго рядом со скриптом (в папке source/)
    ...
    # 2. Проверяем переменную окружения (если задана вручную)
...
def _parse_vless(uri: str) -> Optional[dict]:
    try:
        ...
        # ИСПРАВЛЕНИЕ 2: Удален блок авто-flow, чтобы не ломать TCP+REALITY без Vision
