...
# Источники для ByPassVpnLera.txt (без SNI проверки, только дедупликация)
...
# Добавляем ByPassVpnLera.txt в пути (вместо 26.txt)
    ...
    try:
        ...
        for i, (remote_path, url) in enumerate(zip(REMOTE_PATHS, URLS + [""]), 1):
            ...
            if i <= 25:
                ...
                # Для ByPassVpnLera.txt создаем ссылку на сам файл с текстом "Обход SNI/CIDR белых списков"
            ...
            try:
                ...
                if getattr(e_get, "status", None) == 404:
                    ...
                    if "githubmirror" in remote_path:
                        ...
                        file_index = 26  # Для ByPassVpnLera.txt
...
def create_filtered_configs():
    ...
    # 3. Загрузка доп. источников для ByPassVpnLera.txt + ПОДСЧЕТ УДАЛЕННЫХ
    ...
    extra_configs = []
    total_insecure_filtered_bypass = 0 # Счетчик для ByPassVpnLera.txt
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(4, len(EXTRA_URLS_FOR_BYPASS))) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            total_insecure_filtered_bypass += res_count # Суммируем
    
    # ЕДИНСТВЕННЫЙ ЛОГ ДЛЯ ByPassVpnLera.txt
