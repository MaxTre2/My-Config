...
- ByPassVpnLera.txt с проверкой 300 ключей
...
MAX_KEYS_TO_CHECK = 8000        # Максимум ключей для проверки
TIMEOUT = 12                    # Таймаут 12 секунд
THREADS = 15                    # 15 потоков
...
if hasattr(signal, 'SIGALRM'):
    ...
    signal.alarm(3600)  # 60 минут
    print("✅ Таймаут 60 минут установлен")
...
EXTRA_URLS_FOR_BYPASS = [
    ...
    # XHTTP и XTLS Vision для байпаса
...
def calculate_priority(key: str, latency: int) -> int:
    ...
    # Бонус за XHTTP (минус 100мс к эффективному пингу)
    ...
    if "flow=xtls-rprx-vision" in key.lower():
        priority = max(1, priority - 30)  # Самый быстрый и стабильный протокол
    # =======================================
    
    # Бонус за российский SNI (маскировка под легитимные сервисы)
...
def check_single_key(key: str):
    try:
        ...
        if flow_match:
            flow = unquote(flow_match.group(1))
        # ======================================
        ...
        is_tcp = not (is_ws or is_xhttp)  # Если не ws и не xhttp → tcp
        ...
        # ========== ОСОБАЯ ОБРАБОТКА ДЛЯ REALITY + VLESS + TCP (XTLS Vision) ==========
        if "security=reality" in key and key.startswith("vless://") and is_tcp:
            # REALITY VLESS TCP с XTLS Vision - проверяем только TCP соединение
            # XTLS Vision требует специального рукопожатия, которое мы не эмулируем
    ...
    except (socket.timeout, ssl.SSLError, ConnectionRefusedError, ConnectionResetError):
        return None, None, None, None
    except Exception:
        return None, None, None, None
...
# =============================================================================
# СОЗДАНИЕ БАЙПАС ФАЙЛА С ПРОВЕРКОЙ 300 КЛЮЧЕЙ
# =============================================================================

def _check_single_key_simple(key: str):
    """Быстрая проверка ключа (только TCP соединение)"""
    try:
        if "@" not in key or ":" not in key:
            return False
...
def create_bypass_file():
    """Создает ByPassVpnLera.txt с проверенными рабочими конфигами (300 ключей)"""
    bypass_path = os.path.join(BASE_DIR, "ByPassVpnLera.txt")
    
    print(f"\n🔍 Сбор конфигов для ByPassVpnLera.txt...")
    ...
    print(f"📊 Собрано уникальных конфигов: {len(unique_configs)}")
    
    # Проверка только 300 ключей (для скорости)
    print(f"🔍 Проверка {min(300, len(unique_configs))} ключей...")
    
    working_configs = []
    max_to_test = min(300, len(unique_configs))
    tested = 0
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        future_to_cfg = {executor.submit(_check_single_key_simple, cfg): cfg 
                         for cfg in unique_configs[:max_to_test]}
        ...
        for future in concurrent.futures.as_completed(future_to_cfg):
            ...
            try:
                ...
                if result:
                    ...
                    if len(working_configs) % 50 == 0:
                        print(f"  ✅ Найдено рабочих: {len(working_configs)}/{tested}")
            except Exception:
                pass
    
    print(f"📊 Найдено рабочих конфигов: {len(working_configs)}")
    
    # Берем топ-200 для стабильности
    top_configs = working_configs[:200]
    
    # ========== СОХРАНЕНИЕ ==========
    title = "MaxTre - VPN Bypass (проверенные)"
    title_base64 = base64.b64encode(title.encode('utf-8')).decode('utf-8')
    ...
    print(f"📁 Создан {bypass_path} с {len(top_configs)} проверенными конфигами")
    return bypass_path
...
def main(dry_run=False):
    ...
    # ========== ПРИОРИТИЗАЦИЯ XHTTP + REALITY + XTLS VISION ==========
    ...
    if not dry_run:
        ...
        # =========================================================================
        # WHITE / BLACK сплит (проверяем только топ-200)
        # =========================================================================
        print(f"\n🔬 WHITE / BLACK сплит...")
        if WHITE_CHECK_AVAILABLE and XRAY_AVAILABLE:
            # Проверяем только топ MAX_WHITE_TEST ключей (200)
            ...
            # Непроверенные ключи отправляем в BLACK
