...
- УЛУЧШЕНИЯ: Ротация User-Agent, настраиваемый BYPASS_TEST_LIMIT, XTLS Vision в white_checker
"""
...
MAX_KEYS_TO_CHECK = 4000        # Снижено до безопасного предела (из 8000)
TIMEOUT = 12                    # Таймаут 12 секунд
THREADS = 25                    # Увеличено потоков для компенсации (из 15)
...
BYPASS_TEST_LIMIT = 300         # По умолчанию, можно изменить через --bypass-limit

# =============================================================================
# РОТАЦИЯ USER-AGENT (УЛУЧШЕНИЕ 1)
# =============================================================================
...
USER_AGENTS = [
    ...
    "Mozilla/5.0 (Macintosh; Intel Mac OS X 14_4_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36",
    # Firefox на Windows
    ...
    # Мобильные (для обхода мобильных версий сайтов)
...
# Защита от зависания (только для Unix) - Увеличен таймаут до 90 минут
...
if os.path.exists("../xray"):
    try:
        os.chmod("../xray", 0o755)
        XRAY_AVAILABLE = True
        print("✅ xray найден в корне репозитория")
    except:
        print("⚠️ Не удалось установить права на xray")
elif os.path.exists("xray"):
    try:
        os.chmod("xray", 0o755)
        XRAY_AVAILABLE = True
        print("✅ xray найден рядом со скриптом")
    except:
        print("⚠️ Не удалось установить права на xray")
else:
...
# =============================================================================
# HTTP клиент с ротацией User-Agent (УЛУЧШЕНИЕ 1)
# =============================================================================
...
def _build_session(max_pool_size: int):
    ...
    # Начальный User-Agent, будет меняться при каждом запросе
    session.headers.update(get_rotated_headers())
    return session

REQUESTS_SESSION = _build_session(50)

def fetch_data(url, timeout=10, max_attempts=3):
    """Загрузка данных с ротацией User-Agent"""
    for attempt in range(1, max_attempts + 1):
        try:
            modified_url = url
            verify = True
            
            # Меняем User-Agent перед каждой попыткой
            REQUESTS_SESSION.headers.update(get_rotated_headers())
            ...
            if attempt == max_attempts:
                # Последняя попытка — пробуем с мобильным User-Agent
                try:
                    REQUESTS_SESSION.headers.update({
                        "User-Agent": "Mozilla/5.0 (Linux; Android 14; SM-S928B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Mobile Safari/537.36"
                    })
                    response = REQUESTS_SESSION.get(modified_url, timeout=timeout, verify=False)
                    response.raise_for_status()
                    return response.text
                except:
                    pass
...
def check_single_key(key: str):
    """Проверка одного ключа с поддержкой XTLS Vision (УЛУЧШЕНИЕ 3)"""
    try:
        ...
        # Определяем flow для XTLS Vision
        flow_match = re.search(r'flow=([^&]+)', key)
        flow = unquote(flow_match.group(1)) if flow_match else ""
        has_xtls_vision = "xtls-rprx-vision" in flow
        ...
        # REALITY + VLESS + TCP (включая XTLS Vision)
        if "security=reality" in key and key.startswith("vless://") and is_tcp:
            # Для XTLS Vision пробуем отправить специальный заголовок
            if has_xtls_vision:
                try:
                    ...
                    with socket.create_connection((host, port), timeout=TIMEOUT) as sock:
                        with ctx.wrap_socket(sock, server_hostname=host) as ssock:
                            # XTLS Vision требует отправки специального байта
                            # Отправляем "магический" байт для инициализации Vision
                            ssock.send(b'\x16\x03\x01\x00\xf9\x01\x00\x00\xf5\x03\x03')
                            ssock.settimeout(3)
                            ssock.recv(1)
                except:
                    pass
            else:
                # Обычный REALITY — только TCP
...
def create_bypass_file(bypass_limit: int = BYPASS_TEST_LIMIT):
    ...
    print(f"   Лимит проверки: {bypass_limit} ключей")
    ...
    max_to_test = min(bypass_limit, len(unique_list))
    ...
    header += f"# Лимит проверки: {bypass_limit} | Рабочих: {len(top_configs)}\n"
...
def main(dry_run=False, bypass_limit: int = BYPASS_TEST_LIMIT):
    print("=" * 60)
    print("  🚀 MAIN.PY ULTIMATE (REALITY + XHTTP + XTLS Vision + UA Rotator)")
    print(f"  📊 BYPASS лимит: {bypass_limit} ключей")
    print("=" * 60)
    ...
    if not dry_run:
        ...
        # Используем переданный bypass_limit
        bypass_path = create_bypass_file(bypass_limit=bypass_limit)
        ...
        print(f"  BYPASS    : {bypass_count} (проверено {bypass_limit})")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="MaxTre VPN Config Collector")
    parser.add_argument("--dry-run", action="store_true", help="Только проверка без сохранения")
    parser.add_argument("--bypass-limit", type=int, default=BYPASS_TEST_LIMIT, 
                       help=f"Количество ключей для проверки в ByPassVpnLera.txt (по умолчанию: {BYPASS_TEST_LIMIT})")
    args = parser.parse_args()
    main(dry_run=args.dry_run, bypass_limit=args.bypass_limit)