...
def create_filtered_configs():
    ...
    # ============ АВТОМАТИЧЕСКИЙ ЗАГОЛОВОК ДЛЯ HAPP ============
    local_path_bypass = "ByPassVpnLera.txt"
    try:
        # Первая строка - только название (без #, без лишних символов)
        header = "MaxTre - VPN\n"
        
        with open(local_path_bypass, "w", encoding="utf-8") as file:
            file.write(header + "\n".join(unique_configs))
        log(f"📁 Создан файл {local_path_bypass} с заголовком 'MaxTre - VPN' в первой строке")
    except Exception as e:
