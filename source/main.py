...
# ============ ИСПРАВЛЕННЫЕ ИСТОЧНИКИ (только рабочие) ============
...
def test_config(config_str, timeout=5):
    """Проверяет, работает ли конфиг (TCP connect + задержка)"""
    try:
        ...
        if result == 0:
            # Порт открыт - считаем конфиг рабочим
            return True, latency
        else:
            return False, 9999
    except Exception as e:
...
def create_filtered_configs():
    ...
    log(f"🔄 Тестирование {len(all_configs)} конфигов...")
    
    # Ограничиваем количество тестируемых конфигов
    max_to_test = min(300, len(all_configs))
    test_configs = all_configs[:max_to_test]
    
    working_configs = []
    tested = 0
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
        futures = {executor.submit(test_config, cfg): cfg for cfg in test_configs}
        
        for future in concurrent.futures.as_completed(futures):
            cfg = futures[future]
            tested += 1
            try:
                is_working, latency = future.result(timeout=10)
                if is_working:
                    working_configs.append((cfg, latency))
                    if latency > 0 and latency < 200:
                        log(f"⚡⚡⚡ СУПЕР БЫСТРЫЙ #{tested}: {latency:.0f}ms - {cfg[:60]}...")
                    elif latency > 0 and latency < 500:
                        log(f"⚡ БЫСТРЫЙ #{tested}: {latency:.0f}ms - {cfg[:60]}...")
                    elif latency > 0:
                        log(f"✅ Рабочий #{tested}: {latency:.0f}ms - {cfg[:60]}...")
                    else:
                        log(f"✅ Рабочий #{tested}: (задержка неизвестна) - {cfg[:60]}...")
                else:
                    log(f"❌ Мертвый #{tested}: {cfg[:60]}...")
            except Exception as e:
                log(f"⚠️ Ошибка теста #{tested}: {e}")
    
    # Сортируем по задержке (от быстрых к медленным)
    working_configs.sort(key=lambda x: x[1])
    
    # Отбираем лучшие конфиги (максимум 50)
    best_configs = working_configs[:50]
    
    log(f"📊 Найдено {len(working_configs)} рабочих конфигов")
    log(f"📊 Отобрано {len(best_configs)} лучших для итогового файла")
    
    # Показываем топ-5 самых быстрых
    for i, (cfg, latency) in enumerate(best_configs[:5]):
        if latency > 0:
            log(f"🏆 ТОП-{i+1}: {latency:.0f}ms - {cfg[:80]}...")
    
    all_configs = [cfg for cfg, _ in best_configs]
    # ========================================================
    ...
    try:
        with open(local_path_bypass, "w", encoding="utf-8") as file:
            file.write("\n".join(unique_configs))
        log(f"📁 Создан файл {local_path_bypass} с {len(unique_configs)} лучшими конфигами")
    except Exception as e:
