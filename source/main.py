...
REPO_NAME = "MaxTre2/My-Config"  # ИСПРАВЛЕНО: название вашего репозитория
...
# Источники для ByPassVpnLera.txt (только проверенные)
...
# Best-effort fetch tuning для ByPassVpnLera.txt
...
# Добавляем ByPassVpnLera.txt в пути
...
def create_filtered_configs():
    ...
    try:
        ...
        if '?' in main_part:
            ...
            # Обрабатываем комментарий (часть после #)
            if comment:
                # Декодируем комментарий
                comment = html.unescape(comment)
                comment = urllib.parse.unquote(comment)
                
                # Удаляем [openproxylist.com] и похожие надписи
                ...
                # Если после очистки остался комментарий, добавляем его
...
def main(dry_run: bool = False):
    ...
    # Создаем ByPassVpnLera.txt с отфильтрованными конфигами
    local_path_bypass = create_filtered_configs()
    
    # Загружаем ByPassVpnLera.txt в GitHub
    if not dry_run:
