"""
main.py — ФИНАЛЬНАЯ ВЕРСИЯ (ИСПРАВЛЕННАЯ ВЕРСИЯ)
...
- ИСПРАВЛЕНО: Фейковые TCP-пинги больше не подменяют реальную скорость
- ИСПРАВЛЕНО: ПАРАЛЛЕЛЬНАЯ глубокая Xray-проверка (отсеивает 99% мусора)
- ИСПРАВЛЕНО: Кэш не отравляется ложными срабатываниями
...
# =============================================================================
# Приоритеты (используется только для уже проверенных Xray)
# =============================================================================
...
def check_single_key(key: str, ip_country_map: dict = None):
    try:
        ...
        # ИСПРАВЛЕНИЕ: Просто проверяем, открыт ли порт (TCP Connect).
        # Больше не пытаемся читать 1 байт или эмулировать WS/TLS, что давало фейковые пинги.
        start = time.time()
...
# =============================================================================
# ИСПРАВЛЕННАЯ ПАРАЛЛЕЛЬНАЯ XRAY-ПРОВЕРКА (Реальный трафик)
# =============================================================================
...
def xray_fast_check(keys: list, limit: int = 300) -> list:
    ...
    def test_single_xray(key):
        ...
        try:
            ...
            if proc.poll() is not None:
                raise Exception("Xray упал")

            proxies = {"http": f"socks5h://127.0.0.1:{socks_port}", "https": f"socks5h://127.0.0.1:{socks_port}"}
            
            # ИСПРАВЛЕНИЕ: Замеряем РЕАЛЬНЫЙ пинг только во время HTTP запроса
            start_req = time.time()
            resp = REQUESTS_SESSION.get("http://www.gstatic.com/generate_204", proxies=proxies, timeout=6)
            real_latency = int((time.time() - start_req) * 1000)
            
            if resp.status_code in (204, 200):
                # Извлекаем страну из старого тега, чтобы пересобрать ключ с реальным пингом
    ...
    # Запускаем 15 параллельных проверок Xray
    with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
    ...
    # Возвращаем проверенные + те, что не вошли в лимит (без гарантии)
    return verified_keys + keys[limit:]
...
def create_bypass_file(ip_country_map: dict = None):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=THREADS) as executor:
        future_to_cfg = {executor.submit(check_single_key, cfg, ip_country_map): cfg for cfg in unique_list[:max_to_test]}
...
def main(dry_run=False):
    print("=" * 60)
    print("  🚀 MAIN.PY ФИНАЛЬНАЯ ВЕРСИЯ (REALITY + XHTTP + XTLS Vision)")
    print("=" * 60)
    ...
    # Собираем IP для тех, кто остался UNKNOWN
    unknown_ips = set()
    ...
    for key in all_items:
        ...
        # ИСПРАВЛЕНИЕ: Доверяем кэшу ТОЛЬКО если конфиг прошел через Xray
    ...
    if to_check:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=THREADS) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_key):
                ...
                # ИСПРАВЛЕНИЕ: Сохраняем в кэш с пометкой xray_verified=False
                history[k_id] = {
                    "alive": True, 
                    "latency": latency, 
                    "time": time.time(), 
                    "country": country, 
                    "host": host,
                    "xray_verified": False  # Этот флаг заставит скрипт прогнать его через Xray при следующем запуске, если он не попал в FAST
                }
    ...
    # Сортируем по TCP пингу для передачи в Xray (самые быстрые TCP = самые быстрые Xray обычно)
    ...
    # Отрезаем лимит для глубокой проверки
    res_ru_fast = res_ru_clean[:FAST_LIMIT]
    res_euro_fast = res_euro_clean[:FAST_LIMIT]
    
    # ИСПРАВЛЕНИЕ: Глубокая ПАРАЛЛЕЛЬНАЯ Xray-проверка
    if not dry_run:
        res_ru_fast = xray_fast_check(res_ru_fast, limit=300)
        res_euro_fast = xray_fast_check(res_euro_fast, limit=300)
        
        # Обновляем кэш для тех, кто выжил в Xray (у них теперь актуальные теги с реальным пингом)
        for k in res_ru_fast[:300] + res_euro_fast[:300]:
            k_id = k.split("#")[0]
    ...
    if not dry_run:
        ...
        # Сохраняем обновленный кэш с xray_verified метками
        save_json(HISTORY_FILE, {k: v for k, v in history.items() if now - v["time"] < MAX_HISTORY_AGE})
