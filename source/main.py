...
EXTRA_URLS_FOR_BYPASS = [
    ...
    # Дополнительные источники VLESS
    ...
    # Источники с реальными конфигами
    "https://raw.githubusercontent.com/mehdimoghadasian/v2ray-configs/main/vless.txt",
    "https://raw.githubusercontent.com/ALIILAPRO/v2rayNG-Config/main/vless.txt",
    "https://raw.githubusercontent.com/sarinaesmailzadeh/V2ray-Configs/main/vless.txt",
    
    # Telegram каналы (через GitHub зеркала)
    "https://raw.githubusercontent.com/Arashdadmehr/v2ray-configs/main/vless.txt",
    "https://raw.githubusercontent.com/mostafasadeghi64/V2ray-Config/main/vless.txt",
]
...
# ============ НОВАЯ ФУНКЦИЯ ТЕСТИРОВАНИЯ ============
def test_config(config_str, timeout=5):
    """Проверяет, работает ли конфиг и измеряет пинг"""
    try:
        ...
        if result == 0:
            # Порт открыт, пробуем измерить пинг
...
def create_filtered_configs():
    ...
    working_configs = []
    tested = 0
    max_to_test = min(100, len(all_configs))  # Тестируем максимум 100 конфигов
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            tested += 1
            try:
                is_working, ping_ms = future.result(timeout=10)
                if is_working:
                    working_configs.append((cfg, ping_ms))
                    log(f"✅ Рабочий конфиг #{tested}: {cfg[:80]}... (ping: {ping_ms}ms)")
                else:
                    log(f"❌ Мертвый конфиг #{tested}: {cfg[:80]}...")
            except Exception as e:
                log(f"⚠️ Ошибка при тестировании #{tested}: {e}")
    ...
    # Берем только рабочие конфиги
    all_configs = [cfg for cfg, _ in working_configs]
    # ================================================
    ...
    try:
        with open(local_path_bypass, "w", encoding="utf-8") as file:
            file.write("\n".join(unique_configs))
        log(f"📁 Создан файл {local_path_bypass} с {len(unique_configs)} рабочими конфигами")
    except Exception as e:
