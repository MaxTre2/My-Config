...
- НОВОЕ: Параллельный фетч, GeoIP через ip-api.com, безопасная фильтрация мусора
"""
...
def is_garbage_text(key_str: str) -> bool:
    # ИСПРАВЛЕНИЕ: Ищем мусор СТРОГО в теге (после #), чтобы не убить UUID и Path
    fragment = ""
    if "#" in key_str:
...
# =============================================================================
# ОПРЕДЕЛЕНИЕ СТРАНЫ (ИСПРАВЛЕНО: TLD + TAG)
# =============================================================================

def get_country_fast(host: str, key_name: str) -> str:
    try:
        h = host.lower()
        # Ищем страны только в теге
...
# =============================================================================
# НОВОЕ: МАССОВАЯ GEOIP РЕЗОЛВКА IP-АДРЕСОВ
# =============================================================================

def resolve_ip_countries(ips: set) -> dict:
    """Получает страны для списка IP через ip-api.com/batch (до 100 за раз)"""
    if not ips:
        return {}
    ...
    for i in range(0, len(ip_list), 100):
        ...
        try:
            ...
            # ip-api.com free tier: 15 req/min для HTTP. Ждем 4 секунды между батчами.
...
# =============================================================================
# ЗАГРУЗКА КЛЮЧЕЙ (ИСПРАВЛЕНО: ПАРАЛЛЕЛЬНЫЙ ФЕТЧ)
# =============================================================================

def _process_url(url):
    """Вспомогательная функция для параллельной загрузки"""
    try:
        ...
        for line in lines:
            ...
            if line.startswith(("vless://", "vmess://", "trojan://", "ss://")):
                if not is_garbage_text(line): # Отсеиваем мусор на этапе загрузки
                    added.append(line)
        return added
    except Exception:
        return []
...
def check_single_key(key: str, ip_country_map: dict = None):
    try:
        ...
        # ИСПРАВЛЕНИЕ: Сначала пробуем быстрый парсинг, потом ищем в GeoIP карте
...
def main(dry_run=False):
    ...
    # ИСПРАВЛЕНИЕ: Собираем IP для тех, кто остался UNKNOWN
    unknown_ips = set()
    for key in all_items:
        try:
            ...
            if host and get_country_fast(host, key) == "UNKNOWN":
                # Проверяем, что это именно IP, а не домен
    ...
    for key in all_items:
        ...
        # Определяем страну с учетом новой карты
        country = get_country_fast(host, key)
        ...
        if cached and (current_time - cached["time"] < CACHE_HOURS * 3600) and cached.get("alive"):
            cached_country = cached.get("country", country)
