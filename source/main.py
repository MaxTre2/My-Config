...
# Добавляем новый файл для XHTTP+Reality конфигов
...
def update_readme_table():
    try:
        ...
        # Обновляем таблицу для всех файлов, включая XHTTP_Reality.txt
        for i, (remote_path, url) in enumerate(zip(REMOTE_PATHS, URLS + [""] * 2), 1):
            if i <= 48:
                filename = f"{i}.txt"
                ...
                filename = "XHTTP_Reality.txt"
                raw_file_url = f"https://github.com/{REPO_NAME}/raw/refs/heads/main/XHTTP_Reality.txt"
                source_name = "XHTTP+Reality быстрые конфиги"
                source_column = f"[{source_name}]({raw_file_url})"
            ...
            if i in updated_files:
                ...
                if i <= 48:
                    ...
                    pattern = r"\|\s*49\s*\|\s*\[`ByPassVpnLera\.txt`\].*?\|.*?\|\s*(.*?)\s*\|\s*(.*?)\s*\|"
                else:
                    pattern = r"\|\s*50\s*\|\s*\[`XHTTP_Reality\.txt`\].*?\|.*?\|\s*(.*?)\s*\|\s*(.*?)\s*\|"
...
# ============ НОВЫЕ ФУНКЦИИ ДЛЯ XHTTP+REALITY ============
def generate_reality_keys():
    """Генерирует пару ключей для Reality"""
    try:
        # Пытаемся использовать xray если доступен
    ...
    # Заглушка для тестирования (генерируем случайные)
    # В реальности лучше установить xray на сервере
    chars = string.ascii_letters + string.digits + '-_'
    private_key = ''.join(random.choices(chars, k=44))
    public_key = ''.join(random.choices(chars, k=44))
    return private_key, public_key
...
def create_vless_link(server_ip: str, 
                     port: int, 
                     user_uuid: str, 
                     public_key: str, 
                     short_id: str,
    ...
    # Если путь не стандартный, добавляем его
    if path and path != "/":
        params["path"] = path
    ...
    # Кодируем remark для URL
    remark_encoded = urllib.parse.quote(remark)
    
    # Собираем полную ссылку
    link = f"vless://{user_uuid}@{server_ip}:{port}?{param_string}#{remark_encoded}"
    
    return link

def generate_xhttp_reality_configs():
    """
    Генерирует набор XHTTP+Reality конфигов
    """
    log("🚀 Генерация XHTTP+Reality конфигов...")
    
    # Базовые серверы для генерации
    # В реальности можно брать из существующих рабочих конфигов
    base_servers = [
        {"ip": "5.188.140.104", "port": 443, "country": "🇪🇪 Estonia"},
        {"ip": "109.120.189.243", "port": 52006, "country": "🇫🇮 Finland"},
        {"ip": "212.233.121.53", "port": 443, "country": "🇩🇪 Germany"},
        {"ip": "95.163.210.24", "port": 8083, "country": "🇫🇷 France"},
        {"ip": "158.160.229.58", "port": 443, "country": "🇳🇱 Netherlands"},
    ]
    
    # Разные варианты SNI для маскировки
    sni_options = [
        "yandex.com",
        "google.com",
        "cloudflare.com", 
    ...
    fp_options = ["chrome", "firefox", "edge", "safari", "random"]
    
    # Режимы XHTTP
    xhttp_modes = ["auto", "packet-up", "stream-one", "stream-multi"]
    
    all_links = []
    
    # Генерируем конфиги
    for server in base_servers:
        # Для каждого сервера создаем несколько вариантов
        for sni in random.sample(sni_options, min(3, len(sni_options))):
            for path in random.sample(path_options, 2):
                # Генерируем уникальные ключи для каждого конфига
                ...
                xhttp_mode = random.choice(xhttp_modes)
                
                # Создаем ссылку
                remark = f"{server['country']} | XHTTP+Reality | {sni}"
                link = create_vless_link(
                    server_ip=server["ip"],
                    port=server["port"],
                    user_uuid=user_uuid,
                ...
                # Также создаем вариант с packet-up для скорости
                if random.random() > 0.5:
                    link_fast = create_vless_link(
                        server_ip=server["ip"],
                        port=server["port"],
                        user_uuid=generate_uuid(),
                        public_key=generate_reality_keys()[1],
                        short_id=generate_short_id(),
                        sni=sni,
                        path=path,
                        fp=fp,
                        ...
                        flow="xtls-rprx-vision",
                        remark=f"{server['country']} | XHTTP+Reality FAST | {sni}"
                    )
                    all_links.append(link_fast)
    
    # Тестируем сгенерированные конфиги
    log(f"🔄 Тестирование {len(all_links)} сгенерированных XHTTP+Reality конфигов...")
    
    working_configs = []
    tested = 0
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        futures = {executor.submit(test_config, cfg): cfg for cfg in all_links}
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                if is_working:
                    ...
                    log(f"✅ XHTTP РАБОЧИЙ #{tested}: {cfg[:80]}...")
                else:
                    log(f"❌ XHTTP НЕ РАБОТАЕТ #{tested}: {cfg[:80]}...")
            except Exception as e:
                log(f"⚠️ Ошибка теста XHTTP #{tested}: {e}")
    
    log(f"📊 Найдено {len(working_configs)} рабочих XHTTP+Reality конфигов из {tested} протестированных")
    
    # Если нет рабочих, используем все как запасной вариант
    if len(working_configs) == 0:
        log(f"⚠️ Нет рабочих XHTTP конфигов! Использую все сгенерированные как запасной вариант")
        working_configs = all_links[:50]  # берем первые 50
    ...
    # Сохраняем в файл с заголовком
    local_path = "XHTTP_Reality.txt"
    try:
        # Кодируем название в base64
        title = "MaxTre - XHTTP+Reality"
        ...
        header += f"# Всего конфигов: {len(unique_configs)}\n\n"
        
        with open(local_path, "w", encoding="utf-8") as f:
            f.write(header)
            f.write("\n".join(unique_configs))
        
        log(f"📁 Создан файл {local_path} с {len(unique_configs)} XHTTP+Reality конфигами")
    except Exception as e:
        log(f"⚠️ Ошибка при сохранении {local_path}: {e}")
        return None
...
def create_filtered_configs():
    ...
    # ============ ТЕСТИРОВАНИЕ КОНФИГОВ (пункт 1) ============
    log(f"🔄 Тестирование {len(all_configs)} конфигов на работоспособность...")
    
    working_configs = []
    tested = 0
    max_to_test = min(500, len(all_configs))  # Тестируем максимум 500 конфигов
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        futures = {executor.submit(test_config, cfg): cfg for cfg in all_configs[:max_to_test]}
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                if is_working:
                    working_configs.append(cfg)
                    log(f"✅ РАБОЧИЙ #{tested}: {cfg[:80]}...")
                else:
                    log(f"❌ НЕ РАБОТАЕТ #{tested}: {cfg[:80]}...")
            except Exception as e:
                log(f"⚠️ Ошибка теста #{tested}: {e}")
    
    log(f"📊 Найдено {len(working_configs)} рабочих конфигов из {tested} протестированных")
    
    if len(working_configs) == 0:
        log(f"⚠️ ВНИМАНИЕ: Нет рабочих конфигов! Использую все собранные как запасной вариант")
        all_configs = all_configs[:100]  # берем первые 100 как запасной вариант
    else:
    ...
    # ============ ДОБАВЛЕНИЕ ЗАГОЛОВКА В BASE64 ============
    local_path_bypass = "ByPassVpnLera.txt"
    try:
        # Кодируем название "MaxTre - VPN" в base64
        import base64
        title = "MaxTre - VPN"
        title_bytes = title.encode('utf-8')
        title_base64 = base64.b64encode(title_bytes).decode('utf-8')
        ...
        with open(local_path_bypass, "w", encoding="utf-8") as file:
            file.write(header + "\n".join(unique_configs))
        log(f"📁 Создан файл {local_path_bypass} с base64 заголовком '{title}' и {len(unique_configs)} рабочими конфигами")
    except Exception as e:
...
def main(dry_run=False):
    ...
    # Создаем ByPassVpnLera.txt
    local_path_bypass = create_filtered_configs()
    
    if not dry_run:
        upload_to_github(local_path_bypass, "ByPassVpnLera.txt")
    
    # ============ НОВОЕ: Генерация XHTTP+Reality конфигов ============
    local_path_xhttp = generate_xhttp_reality_configs()
    
    if local_path_xhttp and not dry_run:
        upload_to_github(local_path_xhttp, "XHTTP_Reality.txt")
    # ================================================================
