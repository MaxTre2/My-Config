...
def create_bypass_file():
    ...
    # Дедупликация по k_id (часть до #)
    unique_configs = {}
    for cfg in all_configs:
        k_id = cfg.split("#")[0]
        if k_id not in unique_configs:
        ...
        for future in concurrent.futures.as_completed(future_to_cfg):
            ...
            try:
                ...
                if latency is not None:
                    # Добавляем метку пинга для сортировки
                    flag = country_to_flag(country)
                    k_id = cfg.split("#")[0]
...
def main(dry_run=False):
    ...
    if not dry_run:
        ...
        # Создание ByPassVpnLera.txt с ПОЛНОЙ проверкой
        ...
        print(f"  BYPASS    : {len([c for c in open(bypass_path).readlines() if c.startswith(('vless://', 'vmess://', 'trojan://', 'ss://'))])}")
