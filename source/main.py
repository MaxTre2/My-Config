...
- Сбор из 42 источников
- Продвинутая проверка и классификация
- WHITE/BLACK сплит через white_checker
- ByPassVpnLera.txt с проверкой работоспособности
"""
...
MAX_KEYS_TO_CHECK = 6000        # Уменьшено для стабильности
TIMEOUT = 12                    # Увеличено с 8 (меньше ошибок)
THREADS = 10                    # Уменьшено с 20 (меньше зависаний)
MAX_PING_MS = 5000              # Увеличено с 3000 (включаем медленные)
FAST_LIMIT = 1500               # Топ-1500 быстрых
CHUNK_LIMIT = 1000              
...
if hasattr(signal, 'SIGALRM'):
    ...
    signal.alarm(1500)  # 25 минут
    print("✅ Таймаут 25 минут установлен")
...
# =============================================================================
# СОЗДАНИЕ БАЙПАС ФАЙЛА С ПРОВЕРКОЙ РАБОТОСПОСОБНОСТИ
# =============================================================================
...
def create_bypass_file():
    """Создает ByPassVpnLera.txt с проверенными рабочими конфигами"""
    ...
    print(f"🔍 Проверка {min(1000, len(unique_configs))} ключей...")
    
    working_configs = []
    max_to_test = min(1000, len(unique_configs))
    tested = 0
    ...
    # Берем топ-500 для стабильности
    top_configs = working_configs[:500]
...
def main(dry_run=False):
    print("=" * 60)
    print("  🚀 MAIN.PY ОПТИМИЗИРОВАННАЯ ВЕРСИЯ")
    print("=" * 60)
    ...
    if not dry_run:
        ...
        if WHITE_CHECK_AVAILABLE and XRAY_AVAILABLE:
            ru_white, ru_black = batch_white_check(res_ru_all, history, label="RU")
            euro_white, euro_black = batch_white_check(res_euro_all, history, label="EURO")
        else:
