"""
main.py — ФИНАЛЬНАЯ УСТОЙЧИВАЯ ВЕРСИЯ
...
- ИСПРАВЛЕНО: Фейковые TCP-пинги заменены на ПАРАЛЛЕЛЬНУЮ Xray-проверку
- ИСПРАВЛЕНО: Кэш не отравляется ложными срабатываниями (флаг xray_verified)
- ДОБАВЛЕНО: Защита от падений при отсутствии PyGithub и блокировке файлов
...
# =============================================================================
# БЕЗОПАСНЫЙ ИМПОРТ (Защита от отсутствия PyGithub при локальном тесте)
# =============================================================================
try:
...
# БЕЗОПАСНОЕ определение временной зоны
try:
...
# БЕЗОПАСНАЯ очистка старых папок (защита от заблокированных файлов в Windows/GitHub Actions)
for folder in [FOLDER_RU, FOLDER_EURO]:
    if os.path.exists(folder):
        try:
            shutil.rmtree(folder)
        except Exception as e:
            print(f"⚠️ Не удалось удалить {folder} (пропускаем): {e}")
...
def save_json(path: str, data: dict) -> None:
    try:
        ...
        pass # Тихий сбой кэша не должен ломать скрипт
...
# =============================================================================
# ЗАГРУЗКА В GITHUB (Адаптировано под безопасный импорт)
# =============================================================================
...
def main(dry_run=False):
    print("=" * 60)
    print("  🚀 MAIN.PY ФИНАЛЬНАЯ УСТОЙЧИВАЯ ВЕРСИЯ (REALITY + XHTTP + XTLS Vision)")
    print("=" * 60)
    ...
    if not dry_run:
        ...
        if REPO:
            ...
            print("\n⏭️ Пропуск загрузки на GitHub (токен не настроен или нет библиотеки).")
