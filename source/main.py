...
# Источники для ByPassVpnLera.txt
...
def update_readme_table():
    try:
        ...
        for i, (remote_path, url) in enumerate(zip(REMOTE_PATHS, URLS + [""]), 1):
            filename = f"{i}.txt" if i <= 25 else "ByPassVpnLera.txt"
            ...
            if i in updated_files:
                ...
                if i <= 25:
                    ...
                    pattern = r"\|\s*26\s*\|\s*\[`ByPassVpnLera\.txt`\].*?\|.*?\|\s*(.*?)\s*\|\s*(.*?)\s*\|"
            ...
            if i <= 25:
                ...
                table_rows.append(f"| 26 | [`ByPassVpnLera.txt`]({raw_file_url}) | {source_column} | {update_time} | {update_date} |")
...
def create_filtered_configs():
    ...
    country_names = {
        ...
        'BO': 'Bolivia', 'BQ': 'Caribbean Netherlands', 'BA': 'Bosnia and Herzegovina', 'BW': 'Botswana',
        ...
        'CO': 'Colombia', 'KM': 'Comoros', 'CD': 'Congo (Dem. Rep.)', 'CG': 'Congo (Rep.)',
        ...
        'MF': 'Saint Martin (French)', 'PM': 'Saint Pierre and Miquelon', 'VC': 'Saint Vincent and the Grenadines',
        'WS': 'Samoa', 'SM': 'San Marino', 'ST': 'Sao Tome and Principe', 'SA': 'Saudi Arabia',
        'SN': 'Senegal', 'RS': 'Serbia', 'SC': 'Seychelles', 'SL': 'Sierra Leone', 'SG': 'Singapore',
        'SX': 'Sint Maarten (Dutch)', 'SK': 'Slovakia', 'SI': 'Slovenia', 'SB': 'Solomon Islands',
    ...
    def normalize_vless_link(link):
        """Приводит vless ссылку к стандартному формату и добавляет название страны с [*CIDR]"""
        try:
            ...
            if '?' in main_part:
                ...
                if comment:
                    ...
                    # Удаляем [openproxylist.com] и [openhypoxlist.com]
                    comment = re.sub(r'\[?openproxylist\.com\]?\s*', '', comment, flags=re.IGNORECASE)
                    comment = re.sub(r'\[?openhypoxlist\.com\]?\s*', '', comment, flags=re.IGNORECASE)
                    
                    # Удаляем OpenRay и Dynamic
                    comment = re.sub(r'OpenRay\s*', '', comment, flags=re.IGNORECASE)
                    comment = re.sub(r'Dynamic-\d+\s*', '', comment, flags=re.IGNORECASE)
                    
                    # Удаляем номера типа #KZ | 91.217.10.166
                    comment = re.sub(r'#\s*[A-Z]{2}\s*\|\s*[\d\.]+\s*', '', comment)
                    
                    # Удаляем @Vip_Security join us - 108
                    comment = re.sub(r'@Vip_Security.*?(?:\d+)?', '', comment, flags=re.IGNORECASE)
                    
                    # Ищем двухбуквенный код страны
                    country_match = re.search(r'([A-Z]{2})(?:\s|$)', comment)
                    if country_match:
                        country_code = country_match.group(1)
                        country_full = country_names.get(country_code, country_code)
                        
                        # Проверяем, есть ли в комментарии "VK" или другие дополнения
                        if 'VK' in comment:
                            normalized += f"#{country_full} [*CIDR] VK"
                        else:
                            normalized += f"#{country_full} [*CIDR]"
                    else:
                        # Если не нашли код страны, очищаем комментарий
    ...
    for cfg in all_configs:
        normalized_configs.append(normalize_vless_link(cfg))
    
    log(f"✅ Нормализация завершена")
    all_configs = normalized_configs
    ...
    try:
        with open(local_path_bypass, "w", encoding="utf-8") as file:
            file.write("\n".join(unique_configs))
        log(f"📁 Создан файл {local_path_bypass} с {len(unique_configs)} конфигами")
    except Exception as e:
