...
def _extract_host_port(line):
    """Извлекает хост и порт из строки конфига (для дедупликации)"""
    if not line:
        return None
...
def create_filtered_configs():
    ...
    log(f"📊 Всего собрано {len(all_configs)} конфигов")
    log(f"ℹ️ Тестирование конфигов отключено, сохраняем все")
    # =================================================
    ...
    # ============ ДОБАВЛЕНИЕ ЗАГОЛОВКА ПОДПИСКИ (ИСПРАВЛЕННЫЙ) ============
    local_path_bypass = "ByPassVpnLera.txt"
    try:
        # Специальный формат для автоматического определения заголовка в Happ
        header = "#profile-title: base64:WSBzdHJlYW0gc2VydmljZSBieSBNYXhUcmUK# MaxTre - VPN\n"
        header += "#profile-update-interval: 9\n"
        header += "# MaxTre - VPN - Автоматическая подписка\n"
        header += "# Доступные сервера: Европа, Россия, США\n"
        header += "# Протокол: VLESS (Reality)\n\n"
        
        with open(local_path_bypass, "w", encoding="utf-8") as file:
            file.write(header + "\n".join(unique_configs))
        log(f"📁 Создан файл {local_path_bypass} с заголовком 'MaxTre - VPN' и {len(unique_configs)} конфигами")
    except Exception as e:
