...
class Settings(BaseSettings):
    ...
    max_keys_to_check: int = 5000
    concurrency: int = 30
    timeout: int = 12
    max_ping_ms: int = 4000
    fast_limit: int = 2000
    chunk_limit: int = 1000
    euro_chunk_limit: int = 500
    bypass_test_limit: int = 400
    max_white_test: int = 250
    cache_hours: int = 6
...
class CollectorOrchestrator:
    ...
    async def _check_all(
        ...
        self.logger.info(f"Checking {len(configs)} configs (concurrency={self.settings.concurrency})...")
        ...
        for coro in asyncio.as_completed([check_one(c) for c in configs]):
            ...
            if checked % 100 == 0:
                self.logger.info(
                    f"  Checked: {checked} | RU={len(ru_results)} EU={len(euro_results)}"
                )
