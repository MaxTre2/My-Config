"""
main.py — MaxTre VPN Config Collector v3.1 (Polished)
Async/await, Pydantic Settings, structured logging, retry, atomic file ops,
proper SSL context, real Xray-Core white checking via thread executor.
"""
...
class ProxyChecker:
    ...
    async def _tcp_check(self, host: str, port: int) -> Optional[int]:
        ...
        except (asyncio.TimeoutError, OSError, ValueError, UnicodeError):
            return None

    async def _tls_check(self, host: str, port: int, sni: Optional[str] = None) -> Optional[int]:
        if not self._is_valid_host(host):
            return None
        if sni and not self._is_valid_host(sni):
            sni = None
        start = time.monotonic()
        try:
            ...
            await w.wait_closed()
            return int((time.monotonic() - start) * 1000)
        except (asyncio.TimeoutError, OSError, ssl.SSLError, ValueError, UnicodeError):
            return None
    ...
    async def check(self, proxy: ProxyConfig) -> Optional[CheckResult]:
        ...
        async with self.semaphore:
            ...
            try:
                ...
                self.logger.debug(f"Skipping {proxy.host}: {e}")
                return None
...
class CacheManager:
    ...
    async def save(self) -> None:
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(None, shutil.move, str(tmp), str(self.cache_file))
            self._data = cleaned
        except Exception as e:
...
class GitHubUploader:
    ...
    async def upload(self, local_path: Path, remote_path: str, message: str) -> bool:
        ...
        return await loop.run_in_executor(None, self._upload_sync, local_path, remote_path, message)
            ...
            try:
                try:
                    ...
                    if status == 404:
                        ...
                        self.logger.warning(f"Conflict on {remote_path}, retrying in 1s...")
                        time.sleep(1)
                        continue
                    else:
                        raise
...
class FileGenerator:
    ...
    async def _write_file(self, path: Path, lines: List[str], title: str) -> None:
        header = ""
        if title:
        ...
        await asyncio.get_event_loop().run_in_executor(None, shutil.move, str(tmp), str(path))

    async def save_exact(self, keys: List[str], folder: Path, filename: str, title: str) -> Path:
        folder.mkdir(parents=True, exist_ok=True)
        path = folder / filename
        await self._write_file(path, [k for k in keys if k and k.strip()], title)
...
class BypassGenerator:
    ...
    def _extract_lines(self, raw: str) -> List[str]:
        if "://" not in raw[:200]:
            lines = self._try_decode_base64(raw)
        else:
            lines = [l.strip() for l in raw.splitlines() if l.strip()]
        
        filtered = []
        for line in lines:
            processed = urllib.parse.unquote(line)
            if self.INSECURE_RE.search(processed):
                continue
...
class CollectorOrchestrator:
    GOOD_SNI = frozenset({
        "m.vk.com", "gosuslugi.ru", "sberbank.ru", 
        "yandex.ru", "mail.ru", "ads.x5.ru"
    })
    ...
    @staticmethod
    def calculate_priority(proxy: ProxyConfig, latency: int) -> int:
        priority = latency
        net = proxy.raw_params.get("type", proxy.raw_params.get("net", "")).lower()
        security = proxy.raw_params.get("security", "").lower()
        flow = proxy.raw_params.get("flow", "").lower()
        sni = proxy.raw_params.get("sni", "").lower()
        ...
        if sni in CollectorOrchestrator.GOOD_SNI:
            priority = max(1, priority - 30)
    ...
    async def _check_all(
        ...
        ru_results.sort(key=lambda x: self.calculate_priority(x.proxy, x.latency_ms))
        euro_results.sort(key=lambda x: self.calculate_priority(x.proxy, x.latency_ms))
    ...
    async def run(self) -> None:
        ...
        white_checker = wc.WhiteChecker(
            xray_path=str(self.settings.xray_path) if self.settings.xray_path.exists() else None,
        ...
        async with AsyncHTTPClient(self.settings.timeout, self.logger) as client:
            ...
            await gen.save_chunked(ru_fast, self.settings.folder_ru, "ru_white", self.settings.chunk_limit,
                             "MaxTre - VPN RUSSIA FAST ⚡ Part {idx}")
            self.logger.info("Saving EU FAST...")
            await gen.save_chunked(eu_fast, self.settings.folder_euro, "my_euro", self.settings.euro_chunk_limit,
                             "MaxTre - VPN EUROPE FAST ⚡ Part {idx}")
            self.logger.info("Saving RU ALL...")
            await gen.save_chunked(ru_all, self.settings.folder_ru, "ru_white_all", self.settings.chunk_limit,
                             "MaxTre - VPN RUSSIA ALL 🇷🇺 Part {idx}")
            self.logger.info("Saving EU ALL...")
            await gen.save_chunked(eu_all, self.settings.folder_euro, "my_euro_all", self.settings.euro_chunk_limit,
                             "MaxTre - VPN EUROPE ALL 🇪🇺 Part {idx}")

            # 5. WHITE / BLACK split через executor
            ...
            if not self.settings.dry_run:
                if white_checker.xray_available():
                    ...
                    loop = asyncio.get_event_loop()
                    ru_white, ru_black = await loop.run_in_executor(
                        None,
                        partial(
                    ...
                    euro_white, euro_black = await loop.run_in_executor(
                        None,
                        partial(
                            white_checker.batch_white_check,
                            eu_test,
                ...
                await gen.save_exact(ru_white, self.settings.folder_ru, "ru_white_all_WHITE.txt",
                               "MaxTre - VPN RUSSIA WHITE ✅")
                await gen.save_exact(ru_black, self.settings.folder_ru, "ru_white_all_BLACK.txt",
                               "MaxTre - VPN RUSSIA BLACK ⚠️")
                await gen.save_exact(euro_white, self.settings.folder_euro, "my_euro_all_WHITE.txt",
                               "MaxTre - VPN EUROPE WHITE ✅")
                await gen.save_exact(euro_black, self.settings.folder_euro, "my_euro_all_BLACK.txt",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MaxTre VPN Config Collector v3.1")
    ...
    logger.info("=" * 60)
    logger.info("VPN Collector v3.1 starting")
