...
# =============================================================================
# ИСТОЧНИКИ (включая REALITY и XHTTP)
# =============================================================================
...
def calculate_priority(key: str, latency: int) -> int:
    ...
    good_sni = ["m.vk.com", "gosuslugi.ru", "sberbank.ru", "yandex.ru", "mail.ru"]
    for sni in good_sni:
...
def check_single_key(key: str):
    try:
        ...
        # XHTTP тоже использует HTTP/2 подобное поведение, проверяем как TCP
        ...
        if is_ws:
            ...
            # XHTTP проверяем через TCP + TLS если нужно
            if is_tls:
...
def main(dry_run=False):
    print("=" * 60)
    print("  🚀 MAIN.PY ОПТИМИЗИРОВАННАЯ ВЕРСИЯ (с REALITY + XHTTP)")
    print("=" * 60)
    ...
    # ========== НОВОЕ: ПРИОРИТИЗАЦИЯ XHTTP + REALITY ==========
