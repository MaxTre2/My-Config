...
# white_checker.py должен лежать рядом
        ...
        async with AsyncHTTPClient(self.settings.timeout, self.logger) as client:
            ...
            if not self.settings.dry_run:
                ...
                if white_checker.is_available():
                    ...
                    ru_white, ru_black = await white_checker.batch_check(
                        ru_test, cache=cache._data, cache_hours=24
                    )
                    euro_white, euro_black = await white_checker.batch_check(
                        eu_test, cache=cache._data, cache_hours=24
                    )
