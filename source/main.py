...
class Settings(BaseSettings):
    ...
    max_keys_to_check: int = 2500   # ↓ было 5000 → в 2 раза быстрее
    concurrency: int = 60           # ↑ было 30  → параллелизм
    timeout: int = 6                # ↓ было 12  → быстрый отсев мёртвых
    max_ping_ms: int = 3000         # ↓ было 4000 → только быстрые
    fast_limit: int = 1500
    chunk_limit: int = 1000
    euro_chunk_limit: int = 500
    bypass_test_limit: int = 250    # ↓ было 400 → экономим время
    max_white_test: int = 150       # ↓ было 250 → xray-проверка ~10 мин
    cache_hours: int = 6
...
class ProxyChecker:
    ...
    async def _tls_check(self, host: str, port: int, sni: Optional[str] = None) -> Optional[int]:
        ...
        if not self._is_valid_host(effective_sni):
            effective_sni = host
        start = time.monotonic()
        try:
            ...
            r, w = await asyncio.wait_for(
                asyncio.open_connection(host, port, ssl=ctx, server_hostname=effective_sni),
                timeout=self.settings.timeout,
            )
            w.close()
            await w.wait_closed()
            ...
            # Для REALITY — TLS всё равно откажет, проверим TCP
            return None

    async def _tcp_check_with_fallback(self, proxy: ProxyConfig) -> Optional[int]:
        """Для REALITY: сначала TLS (по SNI), потом TCP."""
        sni = proxy.get_sni()
        lat = await self._tls_check(proxy.host, proxy.port, sni)
        if lat is None:
            # REALITY использует TLS снаружи, но проверяем TCP напрямую
            lat = await self._tcp_check(proxy.host, proxy.port)
        return lat
    ...
    async def check(self, proxy: ProxyConfig) -> Optional[CheckResult]:
        ...
        async with self.semaphore:
            ...
            try:
                net = proxy.get_network()
                security = proxy.get_security()
                sni = proxy.get_sni() or proxy.host

                if proxy.is_reality():
                    # REALITY: проверяем TCP (TLS-握手 будет только в xray)
                    latency = await self._tcp_check_with_fallback(proxy)
                elif proxy.is_xhttp():
                    # SplitHTTP: TLS если порт 443
                    if proxy.port == 443:
                        latency = await self._tls_check(proxy.host, proxy.port, sni)
                    else:
        ...
        for coro in asyncio.as_completed([check_one(c) for c in configs]):
            if time.monotonic() - t_start > deadline_sec:
                self.logger.warning(
                    ...
                    f"stopping early (RU={len(ru_results)} EU={len(euro_results)})"
                )
                break
